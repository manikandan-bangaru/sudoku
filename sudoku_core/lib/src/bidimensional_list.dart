...
class _OneDBackedBidimensionalList<T> extends BidimensionalList<T> {
  ...
  _OneDBackedBidimensionalList(
      {@required List<T> underlyingList,
      @required int width,
      @required int height,
  ...
  factory _OneDBackedBidimensionalList.filled(int width, T fill,
      {int height, bool canMutate = false}) {
    height ??= width;
    final list = List<T>.filled(width * height, fill, growable: false);
    return _OneDBackedBidimensionalList(
        underlyingList: list,
        width: width,
        height: height,
        canMutate: canMutate);
  ...
  BidimensionalList<T> toList({bool growable = true}) {
    ...
    return _OneDBackedBidimensionalList(
        underlyingList: list, width: _x, height: _y, canMutate: growable);
  }
  ...
  BidimensionalList<V> castInner<V>() => _OneDBackedBidimensionalList(
      underlyingList: _underlyingList.cast<V>(),
      width: width,
      height: height,
      canMutate: canMutate);
}
...
class _TwoDBackedBidimensionalList<T> extends BidimensionalList<T> {
  ...
  factory _TwoDBackedBidimensionalList(List<List<T>> underlyingList,
  ...
  _TwoDBackedBidimensionalList._(this._underlyingList, this._x, bool canMutate)
      : super._(canMutate);
...
abstract class BidimensionalList<T> extends ListBase<List<T>> {
  ...
  factory BidimensionalList.view(List<T> list, int width, {int height}) =>
      _OneDBackedBidimensionalList(
          underlyingList: list, width: width, height: height, canMutate: false);
  factory BidimensionalList.filled(int width, T fill,
      {int height, bool canMutate = false}) {
    return _OneDBackedBidimensionalList.filled(width, fill,
        height: height, canMutate: canMutate);
  }
  factory BidimensionalList.generate(int width, T Function(int x, int y) gen,
      {int height, bool canMutate = false}) {
    height ??= width;
    final list = List<List<T>>.generate(height,
        (y) => List<T>.generate(width, (x) => gen(x, y), growable: false),
        growable: false);
  ...
  List<List<T>> get columns {
    return Viewer<List<T>>(
        valueGetter: (int i) => getColumn(i),
        valueSetter: (int i, List<T> value) => setColumn(i, value),
        lengthGetter: () => width,
        lengthSetter: (int length) => width = length);
  }
  ...
  Iterable<V> mapInner<V>(V Function(T element) f) =>
      map((e) => e.map<V>(f)).reduce((a, b) => a.followedBy(b));
  ...
  Iterable<T> whereInner(bool Function(T element) test) =>
      map((e) => e.where(test)).reduce((a, b) => a.followedBy(b));
  ...
  List<T> getRow(int i) {
    ...
    return Viewer<T>(
        lengthGetter: () => width,
        valueGetter: (int i) => getValue(i, y),
        valueSetter: (int i, T value) => setValue(i, y, value),
        lengthSetter: (int i) => width = i);
  }

  void setRow(int i, List<T> value) {
    if (value.length != width) {
      throw StateError(
          "You can't set an eow that isn't the same size as the width");
    }
  ...
  List<T> getColumn(int i) {
    ...
    return Viewer<T>(
        lengthGetter: () => height,
        valueGetter: (int i) => getValue(x, i),
        valueSetter: (int i, T value) => setValue(x, i, value),
        lengthSetter: (int i) => height = i);
  }

  void setColumn(int i, List<T> value) {
    if (value.length != height) {
      throw StateError(
          "You can't set an column that isn't the same size as the height");
    }
...
class Viewer<T> extends ListBase<T> {
  ...
  Viewer(
      ...
      @required LengthGetter lengthGetter,
      LengthSetter lengthSetter})
